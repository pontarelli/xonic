...
/*
static struct sk_buff *onic_run_xdp(struct onic_ring  *rx_ring,
				     struct xdp_buff *xdp)
{
...
static int onic_rx_poll(struct napi_struct *napi, int budget)
{
	...
	bool flipped = 0;
	bool debug = 0;
	bool xdp_drop_flag=false;

	//pr_info("Queue id: %u", qid);
	...
	if (debug)
		netdev_info(
			q->netdev,
			"\n (qid %d) rx_poll:  cmpl_stat_pidx %u, color_cmpl_stat %u, cmpl_ring next_to_clean %u, cmpl_stat_cidx %u, intr_state %u, cmpl_ring->count %u (rx_packets %lld)",
			qid,cmpl_stat.pidx, color_stat, cmpl_ring->next_to_clean,
			cmpl_stat.cidx, cmpl_stat.intr_state, cmpl_ring->count,priv->netdev_stats.rx_packets);
	...
	while ((cmpl_ring->next_to_clean != cmpl_stat.pidx)) {
		...
		buf = &q->buffer[desc_ring->next_to_clean];
		prefetchw(page_address(buf->pg)+buf->offset);
		len = cmpl.pkt_len;
		data = (u8 *)(page_address(buf->pg) + buf->offset);

		//XDP processing (from virtio)
		//rcu_read_lock();
		xdp_prog = READ_ONCE(desc_ring->xdp_prog);
		...
		if (xdp_prog) {
			...
			orig_data = xdp.data;
			// the entire bunch of packets is processed by the XDP program
			act = bpf_prog_run_xdp(xdp_prog, &xdp);
			...
			switch (act) {
				...
				case XDP_TX:
					...
					onic_xmit_xdp_frame(q->netdev, data, len);
					xdp_drop_flag = true;
					break ;
					//default:
					//	#if LINUX_VERSION_CODE >= KERNEL_VERSION(6,2,0)
				...
				case XDP_DROP:
					...
					xdp_drop_flag=true;
					if (debug) {
						pr_info("dropped %d pkts\n", desc_ring->xdp_drop);
					}
			}
			//rcu_read_unlock();

			if (!xdp_drop_flag) {
				// sending the packet to the network stack
				if (delta != 0) {
					pr_err("delta != 0 not implemented!!\n");
					rv = -ENOMEM;
					break;
				}
		...
		if (onic_ring_full(desc_ring)) {
			if (debug) netdev_info(q->netdev, "desc_ring full");
		}

		if (onic_rx_high_watermark(q)) {
			if (debug) netdev_info(q->netdev, "High watermark: h = %d, t = %d",
				   desc_ring->next_to_use,
				   desc_ring->next_to_clean);
			onic_rx_refill(q);
		}
		...
		if (onic_ring_full(cmpl_ring)) {
			if (debug) netdev_info(q->netdev, "cmpl_ring full");
		}
		if (cmpl.color != cmpl_ring->color) {
			if (debug)
				netdev_info(
		...
		if ((++work) >= budget) {
			if (debug)
				netdev_info(q->netdev,
					    "(qid %d) watchdog work %u, budget %u", qid, work,
					    budget);
			//https://docs.kernel.org/networking/napi.html
			// The poll method returns the amount of work done. If the driver still has outstanding work to do (e.g. budget was exhausted) the poll method should return exactly budget. 
			// In that case, the NAPI instance will be serviced/polled again (without the need to be scheduled).
			//napi_complete(napi);
	...
	if (cmpl_ring->next_to_clean == cmpl_stat.pidx) {
		...
		if (debug)
			netdev_info(
				q->netdev,
				"(qid %d) next_to_clean == cmpl_stat.pidx %u, napi_complete work %u, budget %u, rval %s",
				qid,cmpl_stat.pidx, work, budget,
				napi_cmpl_rval ? "true" : "false");
		if (napi_cmpl_rval) onic_set_completion_tail(priv->hw.qdma, qid,
					 cmpl_ring->next_to_clean, 1);
		if (debug)
			netdev_info(q->netdev, "onic_set_completion_tail to %u",cmpl_ring->next_to_clean);
	} else if (cmpl_ring->next_to_clean == 0) {
		napi_cmpl_rval = napi_complete_done(napi, work);
		if (debug) {
			netdev_info(
				q->netdev,
				"(qid %d) next_to_clean == 0, napi_complete work %u, budget %u, rval %s",
				qid,work, budget,
				napi_cmpl_rval ? "true" : "false");
			netdev_info(q->netdev,
				    "(qid %d) napi_complete work %u, budget %u, rval %s",
				    qid, work, budget,
				    napi_cmpl_rval ? "true" : "false");
		}
		if(napi_cmpl_rval) onic_set_completion_tail(priv->hw.qdma, qid,
...
out_of_budget:
	if (debug) {
		...
		netdev_info(
			q->netdev,
			"(qid %d) rx_poll returning work %u, rx_packets %lld, rx_bytes %lld",
			qid,work, priv->netdev_stats.rx_packets,
		...
		netdev_info(
			q->netdev,
			"qdma_read_reg: hw_cmpl_stat.cidx %u (%x), dsc.pidx 0x%x",
			(0xffff & qdma_read_reg((struct qdma_dev *)priv->hw.qdma,
			QDMA_OFFSET_DMAP_SEL_CMPL_CIDX + (qid * 16))),
			qdma_read_reg((struct qdma_dev *)priv->hw.qdma,
			QDMA_OFFSET_DMAP_SEL_CMPL_CIDX + (qid * 16)),
			qdma_read_reg((struct qdma_dev *)priv->hw.qdma,
			QDMA_OFFSET_DMAP_SEL_C2H_DESC_PIDX + (qid * 16))
		); 
		netdev_info(
			q->netdev,
			"c2h_cmpl pkt_id %u, pkt_len %u, error %u, color %u cmpl_ring->color:%u",
...
netdev_tx_t onic_xmit_xdp_frame_differed(struct net_device *dev, u16 qid, unsigned char* data, unsigned int len, struct onic_rx_buffer *buf)
{
	struct onic_private *priv = netdev_priv(dev);
	struct onic_tx_queue *q;
	struct onic_ring *ring;
	...
	// swap old buffer page with new buffer page
	buf->pg = q->buffer[ring->next_to_use].pg;
	q->buffer[ring->next_to_use].pg = pg;


	dma_addr = dma_map_single(&priv->pdev->dev, data, len, DMA_TO_DEVICE);
	...
	q->buffer[ring->next_to_use].len = len;

	priv->netdev_stats.tx_packets++;
	priv->netdev_stats.tx_bytes += len;
...
netdev_tx_t onic_xmit_xdp_frame(struct net_device *dev, unsigned char* data, unsigned int len)
{
	struct onic_private *priv = netdev_priv(dev);
	struct onic_tx_queue *q;
	struct onic_ring *ring;
	struct qdma_h2c_st_desc desc;
	u16 qid = 0; //priv->num_tx_queues-1; //skb->queue_mapping;
	dma_addr_t dma_addr;
	u8 *desc_ptr;
	//int rv;
