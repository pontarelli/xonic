...
#ifndef ONIC_VF
#define DRV_STR "OpenNIC Linux Kernel Driver with XDP"
...
struct onic_device_data device_data;
...
static int onic_xdp_setup(struct net_device *dev, struct bpf_prog *prog)
{
        //int i, frame_size = dev->mtu + ETH_HLEN + ETH_FCS_LEN + VLAN_HLEN;
        int i; 
	struct onic_private *adapter = netdev_priv(dev);
	...
	/* verify onic ring attributes are sufficient for XDP */
	//for (i = 0; i < adapter->num_rx_queues; i++) {
		//struct onic_rx_queue *ring = adapter->rx_queue[i]->desc_ring;
	...
	for (i = 0; i < adapter->num_rx_queues; i++) {
		xchg(&adapter->rx_queue[i]->desc_ring.xdp_prog, adapter->xdp_prog);
	}
...
//static u32 onic_xdp_query(struct net_device *dev)
//{
//	struct onic_private *adapter = netdev_priv(dev);
...
//	int i;
	...
	//pr_info("adapter num queues = %d \n",adapter->num_rx_queues);
	//ERROR in adapter->rx_queue[i] --> NULL
	//for (i = 0; i < adapter->num_rx_queues; i++) {
	//	xdp_prog = rtnl_dereference(adapter->rx_queue[i]->desc_ring.xdp_prog);
	//	if (xdp_prog)
...
static int onic_xdp(struct net_device *dev, struct netdev_bpf *xdp)
{
	...
	switch (xdp->command) {
	case XDP_SETUP_PROG:
		return onic_xdp_setup(dev, xdp->prog);
...
static int onic_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	netdev = alloc_etherdev_mq(sizeof(struct onic_private),
				   ONIC_MAX_QUEUES);
	printk("alloc_etherdev_mq max queues: %d\n", ONIC_MAX_QUEUES);
	printk("netdev->num_tx_queues: %d\n", netdev->num_tx_queues);
	printk("netdev->num_rx_queues: %d\n", netdev->num_rx_queues);
	if (!netdev) {
	...
	// queueu is set to the minimum of the number of queues supported by the device and the number of queues requested by the user
	// max number of interrupts available is 8
	...
	if (PCI_FUNC(pdev->devfn)==0) {
		if (onic_create_chrdev(&device_data) != 0) {
			dev_err(&pdev->dev, "Failed to create chrdev");
			goto clear_interrupt;
		}
	}
	//return register_btf_kfunc_id_set(BPF_PROG_TYPE_XDP, &bpf_task_kfunc_set); 
	if (pagepool_init() != 0) {
		dev_err(&pdev->dev, "Failed to create pagepool");
		goto clear_interrupt;
	}
	return 0;
...
#endif
	onic_remove_chrdev(&device_data);
}
...
//__bpf_kfunc int bpf_wrapper_onic_sum(int a, int b)
//{
//	return onic_sum(a, b);
//}
